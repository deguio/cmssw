...
HLTHiggsSubAnalysis::HLTHiggsSubAnalysis(const edm::ParameterSet & pset,
	...
	//_genParticleLabel(consumes<reco::GenParticleCollection>(pset.getParameter<std::string>("genParticleLabel"))),
      	_parametersEta(pset.getParameter<std::vector<double> >("parametersEta")),
  	_parametersPhi(pset.getParameter<std::vector<double> >("parametersPhi")),
  	_parametersTurnOn(pset.getParameter<std::vector<double> >("parametersTurnOn")),
	//_trigResultsTag(consumes<edm::TriggerResults>(edm::InputTag("TriggerResults","",_hltProcessName))),
	_recMuonSelector(0),
	_recElecSelector(0),
